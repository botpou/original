...
async function createBot(sessionId) {
  ...
  try {
    ...
    // Store removeCreds function for later use
    ...
    client.ev.on("connection.update", async (update) => {
      ...
      } else if (qr && !client.authState.creds.registered) {
        // Handle pairing code request and QR code separately
        if (!client.authState.creds.pairingCode) {
          try {
            // Don't save pairing code credentials yet, only after successful pairing
            console.log("Requesting pairing code...");
          } catch (error) {
            console.error("Error requesting pairing code:", error);
            // Clean up any partial credentials if there's an error
            await removeCreds();
          }
        }
      }
    ...
    // This will save credentials when successful authentication happens
  ...
  try {
    ...
    socket.ev.on("connection.update", async update => {
      ...
      if (connection === "close") {
        ...
        await loadPlugins();
        console.log("All plugins installed.");
      } else if (qr && !socket.authState.creds.registered) {
        // Handle pairing code request and QR code separately
        if (!socket.authState.creds.pairingCode) {
          try {
            // Don't save pairing code credentials yet, only after successful pairing
            console.log("Requesting pairing code for restored session...");
          } catch (error) {
            console.error("Error requesting pairing code:", error);
            // Clean up any partial credentials if there's an error
            await removeCreds();
...
app.post("/pairing-code", async (req, res) => {
  try {
    ...
    setTimeout(async () => {
      try {
        // Request pairing code without saving credentials yet
        ...
        // Clean up any partial credentials to allow retrying
        const sessionPath = `./sessions/${phoneNumber}`;
        const credsPath = `${sessionPath}/creds.json`;
        if (fs.existsSync(credsPath)) {
          try {
            fs.unlinkSync(credsPath);
            console.log("Cleaned up partial credentials due to pairing error");
          } catch (err) {
            console.error("Error cleaning up credentials:", err);
          }
        }
